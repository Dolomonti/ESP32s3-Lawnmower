...
// Phase 4.3: SerialCommand, globale Variablen und Steuerungsvariablen sind jetzt in HoverboardComm.h
...
#include <atomic>    // Phase 2: Thread-sichere atomare Variablen
...
HardwareSerial HoverSerialInstance(1);  // Use UART1 for hoverboard communication
extern HardwareSerial* HoverSerial;  // Phase 4.3: Pointer ist in HoverboardComm.cpp definiert
// Phase 4.3: Variablen für Receive() (sind in HoverboardComm.cpp definiert)
extern uint8_t idx;
...
// Phase 4.3: Send() Deklaration ist jetzt in HoverboardComm.h
...
// Phase 4.3: SerialFeedback Structs sind jetzt in HoverboardComm.h
...
extern volatile int16_t feedbackDistR;    // Phase 4.3: In HoverboardComm.cpp definiert
...
// Phase 4.3: bufferMutex und feedbackMutex sind jetzt in HoverboardComm.cpp
// Sie werden in initHoverboardComm() erstellt
...
void setup() {
  ...
  // Serial starten
  // Phase 4.3: HoverboardComm initialisieren
  initHoverboardComm(HoverSerial);
    ...
    for (;;) {
        ...
        */
        ...
        if (ws.count() > 0) {
            DynamicJsonDocument doc(512);
            doc["type"] = "status";
            doc["estop"] = skill8SafetyActive.load(); // Phase 2: Atomar lesen für JSON
            ...
            doc["turn_active"] = skillActive.load(); // Phase 2: Atomar lesen für JSON
